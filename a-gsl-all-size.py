...
results = torch.load('results/gsl-all-size.pt')
root = 'data'
...
for metric_idx, metric in enumerate(['inner', 'outer', 'HP', 'overall']):
    ...
    global_ax.set_xlabel('best beta')
    global_ax.grid(ls=":")
    ...
    for config_idx, (kkc, uuc) in enumerate(tqdm(config)):
        ...
        if toss == repeats-1:
            ...
            aa.set_yticks(np.arange(0,q,step), 
                        ['%.2f' % s for s in sizes.numpy()[::step]])
            ...
            much_sizes = np.sum(best_epsilon_mask, axis=1)
            
            best_sizes = sizes[much_sizes > 0]
            much_sizes = much_sizes[much_sizes > 0]            
            
            global_ax.scatter(best_sizes, 
                              [kkc_idx for v in best_sizes],
                              s=(200*much_sizes/np.sum(much_sizes)), #10+uuc_idx*30, 
