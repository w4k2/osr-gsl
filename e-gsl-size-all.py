...
for config_idx, (kkc, uuc) in enumerate(config):        
        ...
        for fold in range(n_splits):
            ...
            for size_id, size in enumerate(sizes):
                ...
                # Initialize model
                model = GSL(n_known=len(kkc), lower_stack=fc_lower_stack(depth=1, img_size_x=28, n_out_channels=64), sigma=3, n_generated=size, normal=True) 
                ...
                print(results[:, config_idx, fold, size_id])
        
                
                    
            torch.save(results, 'results/gsl-all-size.pt')
